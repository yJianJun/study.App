...
import com.example.agent.autoJS.AutoJsUtil;
import com.example.agent.device.ChangeDeviceInfoUtil;

import com.example.agent.proxy.ClashUtil;
import com.example.agent.service.MyAccessibilityService;
import com.example.agent.utils.LogFileUtil;
import com.example.agent.worker.CheckAccessibilityWorker;
...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (androidId == null || androidId.isEmpty()) {
      LogFileUtil.logAndWrite(Log.ERROR, "MainActivity", "Android ID is empty", null);
      finish();
      return;
    }
    ...
    checkNetworkAndFinishIfNotAvailable();
    ...
    setupUiComponents(androidId, taskId);
  }
  ...
  private void checkNetworkAndFinishIfNotAvailable() {
  ...
  private void setupWorkManager() {
    try {
      ...
      LogFileUtil.logAndWrite(Log.ERROR, "MainActivity", "Failed to setup WorkManager", e);
    }
  }

  private void setupUiComponents(String androidId, String taskId) {
    setButtonClickListener(R.id.run_script_button, v -> AutoJsUtil.runAutojsScript(this));
    setButtonClickListener(R.id.connectVpnButton, v -> startProxyVpn(this));
    setButtonClickListener(R.id.disconnectVpnButton, v -> ClashUtil.stopProxy(this));
    setButtonClickListener(R.id.switchVpnButton, v -> ClashUtil.switchProxyGroup("GLOBAL", "us", "http://127.0.0.1:6170"));
    setButtonClickListener(R.id.modifyDeviceInfoButton, v -> ChangeDeviceInfoUtil.changeDeviceInfo(getPackageName(), this));
    setButtonClickListener(R.id.resetDeviceInfoButton, v -> ChangeDeviceInfoUtil.resetChangedDeviceInfo(getPackageName(), this));

    Button executeButton = findViewById(R.id.execute_button);
    if (executeButton != null) {
      executeButton.setOnClickListener(v -> {
        if (!isExecuteAllowed()) return;
  ...
  private boolean isExecuteAllowed() {
    if (!executeAllowed) {
      Toast.makeText(this, "请勿重复执行", Toast.LENGTH_SHORT).show();
      return false;
    }
  ...
  private void setButtonClickListener(int resId, View.OnClickListener listener) {
    Button button = findViewById(resId);
    if (button != null) {
      button.setOnClickListener(listener);
    } else {
      String message = getResources().getResourceName(resId) + " not found";
